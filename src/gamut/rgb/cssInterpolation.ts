...
export class CSS4GamutMapping extends ToRGBColorspaceVisitor {
  ...
  public visitRGBColor(color: InstanceType<typeof Color.RGB>) {
    ...
    // Step 5 : let inGamut(color) be a function which returns true if, when passed a color, that color is inside the gamut of destination.
    ...
    let current: InstanceType<typeof Color.OkLCH> = origin_Oklch;
